...
import (
	...
	"github.com/anarakinson/go_stonks/stonks_pb/gen/market"
...
func MarketToProto(m *domain.Market) *market_pb.Market {
	AllowedRoles := []market.UserRole{}
	for _, role := range m.AvailableRoles {
		switch role {
		case domain.UserRole_BASIC:
			AllowedRoles = append(AllowedRoles, market.UserRole_ROLE_BASIC)
		case domain.UserRole_PROFESSIONAL:
			AllowedRoles = append(AllowedRoles, market.UserRole_ROLE_PROFESSIONAL)
		case domain.UserRole_WHALE:
			AllowedRoles = append(AllowedRoles, market.UserRole_ROLE_WHALE)
		}
	}
